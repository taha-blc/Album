...
//     // { word: "Orman Meyvesi", hint: "Lamranın en sevdiği meyve" }
// ];
...
async function syncData() {
    try {
        ...
        if (!qErr && questions && questions.length > 0) {
            ...
            console.log("Sorular Supabase'den çekildi.");
        } else {
            // Eğer DB boşsa varsayılan soruları geri getir
            HANGMAN_QUESTIONS = [
                { word: "SU YEŞİLİ", hint: "🎨 Namnanın En Sevdiği Renk" },
                { word: "BEYAZLARI", hint: "Takonun namnada en sevdiği detay" },
                { word: "TİLKİ", hint: "Takoya benzeyen animasyon karakteri" },
        ...
        // 3) Sabit Anıları çek (Eğer memories tablosu dolaysa)
...
function tick() {
    ...
    if (dateEl) dateEl.textContent = d.toLocaleDateString("tr-TR", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
}
...
function typeLoop() {
    ...
    if (!deleting) {
        ...
        if (ci === current.length) { deleting = true; setTimeout(typeLoop, 950); return; }
    } else {
        ci--;
        typingEl.textContent = current.slice(0, ci);
...
if (moodEl) moodEl.textContent = moodOptions[Math.floor(Math.random() * moodOptions.length)] + ` (Tarih: ${new Date().toLocaleDateString("tr-TR")})`;
...
document.getElementById("btnLetter").addEventListener("click", () => {
    openModal("💌 Mektup", `<p class="muted">Lamram,<br><br>Bazen kelimeler yetersiz kalıyor ama yine de kalbim denemek istiyor, Bebeğim...<br><br>Seninle konuşmak ayrı güzel, gülmek ayrı, hatta yan yana susmak bile başka bir huzur.<br>Utandığında dudaklarının aldığı o masum şekil var ya… işte orada dünya bir anlığına duruyor benim için.<br><br>Bazen gözlerini kaçırıyorsun — camdan bakarken kaçırdığına eminim ama ispatlayamam — ama ben her bakışında yeniden yakalanıyorum sana.<br><br>Sana baktığımda içimden hep aynı cümle geçiyor: “Gerçekten böyle güzel bir kadına sahip olabilir miyim?”<br>Ay tenin, gecenin karanlığında en parlak ışığım oluyor.<br>Kokun… dünyadaki en huzurlu yer gibi.<br><br>İyi ki varsın.<br>İyi ki kalbim seni seçmiş.<br>İyi ki benimsin. 💖<br><br><b>— Takoşşş</b></p>`);
});
...
function getAllMemories() {
    const uploaded = UPLOADED_PHOTOS.map(p => ({ title: p.name, text: p.desc, img: p.src }));
...
async function handleFiles(files) {
    ...
    for (const file of arr) {
        ...
        const { data, error } = await supabaseClient.storage.from('fotograflar').upload(fileName, file, { upsert: true });
        if (error) continue;

        const { data: { publicUrl } } = supabaseClient.storage.from('fotograflar').getPublicUrl(fileName);
...
document.getElementById("editSaveBtn").onclick = async () => {
    ...
    const { error } = await supabaseClient.from('uploaded_photos').update({ title: newName, description: newDesc }).eq('id', editingPhotoId);
    if (!error) {
...
document.getElementById("editCancelBtn").onclick = () => document.getElementById("editPhotoModal").classList.remove("show");

function buildMemoriesModal() {
    const staticCards = MEMORIES.map((m, idx) => `
        <div class="gal-item" data-idx="${idx}"><img src="${m.img}" loading="lazy"><div class="gal-overlay"><b>${m.title}</b><span>${m.text}</span></div></div>
    `).join("");
    ...
    const uploadedCards = UPLOADED_PHOTOS.map((p, idx) => `
        <div class="gal-item uploadedPhoto" data-idx="${MEMORIES.length + idx}">
            ...
            <div class="gal-overlay"><b>${p.name}</b><span>${p.desc}</span></div>
        </div>
    `).join("");
    ...
    openModal("📸 Minik Anılar", `
        <div class="upload-zone" id="uploadZone">
            ...
            <div class="upload-label"><b>Fotoğraf Yükle</b><br><span>Supabase Bulutuna Kaydet ☁️</span></div>
        </div>
    ...
    document.querySelectorAll(".gal-item").forEach(el => el.onclick = () => openLightbox(parseInt(el.dataset.idx)));
}
...
document.getElementById("lbPrev").onclick = () => { lbIndex--; updateLightbox(); };
document.getElementById("lbNext").onclick = () => { lbIndex++; updateLightbox(); };

// ==========================================
// 11) Adam Asmaca
// ==========================================
const MAX_WRONG = 6;
let hmWord = "", hmHint = "", hmGuessed = new Set(), hmWrong = 0, hmGameOver = false;
...
function hmDrawSVG(wrong) {
    ...
    const scaffold = `<line x1="30" y1="150" x2="140" y2="150" stroke="rgba(255,255,255,.35)" stroke-width="2.5"/><line x1="60" y1="150" x2="60" y2="10" stroke="rgba(255,255,255,.35)" stroke-width="2.5"/><line x1="60" y1="10" x2="100" y2="10" stroke="rgba(255,255,255,.35)" stroke-width="2.5"/><line x1="100" y1="10" x2="100" y2="31" stroke="rgba(255,255,255,.35)" stroke-width="2.5"/>`;
    return `<svg viewBox="0 0 160 160" xmlns="http://www.w3.org/2000/svg">${scaffold}${parts.slice(0, wrong).join("")}</svg>`;
}

function hmRender() {
    document.getElementById("hmSvg").innerHTML = hmDrawSVG(hmWrong);
    document.getElementById("hmWord").innerHTML = hmWord.split("").map(ch => ch === " " ? `<div style="width:20px;"></div>` : `<div class="hm-letter${hmGuessed.has(ch) ? " revealed" : ""}">${hmGuessed.has(ch) ? ch : ""}</div>`).join("");
    document.getElementById("hmLives").innerHTML = Array.from({ length: MAX_WRONG }, (_, i) => `<span class="hm-life${i >= (MAX_WRONG - hmWrong) ? " gone" : ""}">💖</span>`).join("");
    document.querySelectorAll(".key-btn").forEach(btn => { if (hmGuessed.has(btn.dataset.ch)) { btn.disabled = true; btn.classList.add(hmWord.includes(btn.dataset.ch) ? "correct" : "wrong"); } });

    const allRevealed = hmWord.replace(/ /g, "").split("").every(ch => hmGuessed.has(ch));
    if (allRevealed) { document.getElementById("hmStatus").innerHTML = `<span style="color:#22c55e;">🎉 Kazandın! 🎊</span>`; launchConfetti(); hmGameOver = true; }
    else if (hmWrong >= MAX_WRONG) { document.getElementById("hmStatus").innerHTML = `<span style="color:#ef4444;">💔 Kelime: ${hmWord}</span>`; hmGameOver = true; }
}

function startHangman() {
    const q = HANGMAN_QUESTIONS[Math.floor(Math.random() * HANGMAN_QUESTIONS.length)];
    hmWord = q.word.replace(/i/g, "İ").replace(/ı/g, "I").toUpperCase(); hmHint = q.hint; hmGuessed = new Set(); hmWrong = 0; hmGameOver = false;
    openModal("🎮 Adam Asmaca", `<div class="hangman-wrap"><div class="hm-scene" id="hmSvg"></div><div class="hm-hint">💡 ${hmHint}</div><div class="hm-word" id="hmWord"></div><div class="hm-lives" id="hmLives"></div><div class="hm-status" id="hmStatus"></div><div class="hm-keyboard">${ALPHABET.map(ch => `<button class="key-btn" data-ch="${ch}">${ch}</button>`).join("")}</div><button onclick="startHangman()" class="primary" style="margin-top:15px;">🔄 Yeni Kelime</button></div>`);
    document.querySelectorAll(".key-btn").forEach(btn => btn.onclick = () => { if (!hmGameOver && !hmGuessed.has(btn.dataset.ch)) { hmGuessed.add(btn.dataset.ch); if (!hmWord.includes(btn.dataset.ch)) hmWrong++; hmRender(); } });
    hmRender();
}
...
function launchConfetti() {
    ...
    confetti.innerHTML = ""; confetti.classList.add("show");
    for (let i = 0; i < 130; i++) {
        const p = document.createElement("i");
        p.style.left = Math.random() * 100 + "vw"; p.style.animationDuration = (2 + Math.random() * 2) + "s";
...
let W, H, hearts;
function resize() {
    W = canvas.width = window.innerWidth * devicePixelRatio; H = canvas.height = window.innerHeight * devicePixelRatio;
    hearts = Array.from({ length: 55 }, () => ({ x: Math.random() * W, y: Math.random() * H, s: (0.7 + Math.random() * 1.8) * devicePixelRatio, vy: (0.35 + Math.random() * 1.1) * devicePixelRatio, vx: (-0.25 + Math.random() * 0.5) * devicePixelRatio, a: 0.35 + Math.random() * 0.55 }));
}
window.onresize = resize;
function loop() {
    ctx.clearRect(0, 0, W, H);
    hearts.forEach(h => {
        ctx.save(); ctx.translate(h.x, h.y); ctx.scale(h.s, h.s); ctx.globalAlpha = h.a; ctx.fillStyle = "#ff4d8d";
        ctx.beginPath(); ctx.moveTo(0, 6); ctx.bezierCurveTo(-10, -4, -14, 10, 0, 16); ctx.bezierCurveTo(14, 10, 10, -4, 0, 6); ctx.fill(); ctx.restore();
        h.y += h.vy; h.x += h.vx; if (h.y > H + 20) h.y = -20;
    });
...
// 13) Evet/Hayır & Init
// ==========================================
document.getElementById("btnYes").onclick = () => { const msg = document.getElementById("finalMsg"); msg.style.display = "block"; msg.innerHTML = "O zaman konfetiiiiiiler şana gelsinnnnn 💐✨"; launchConfetti(); };
document.getElementById("btnNo").onmouseenter = function () { this.style.position = "absolute"; this.style.left = Math.random() * 80 + "%"; this.style.top = Math.random() * 80 + "%"; };

syncData().then(() => console.log("Sistem hazır! 🚀"));

// ==========================================
// 14) Fiziksel Klavye Desteği ⌨️
// ==========================================
window.addEventListener("keydown", (e) => {
    // Sadece modal açıksa ve adam asmaca oynanıyorsa çalış
    ...
    if (isModalOpen && isHangmanActive && !hmGameOver) {
        ...
        if (ALPHABET.includes(key)) {
            // Harf daha önce tahmin edilmediyse tahmin et
            if (!hmGuessed.has(key)) {
                hmGuessed.add(key);
                if (!hmWord.includes(key)) hmWrong++;
                hmRender();
            }
        }
    }
